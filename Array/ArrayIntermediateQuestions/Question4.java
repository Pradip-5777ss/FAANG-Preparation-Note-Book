...
public class Question4 {
	...
	public static void main(String[] args) {
		...
		//Ask user to enter the size of the array. initialize the size into the variable name size.
		...
		//Ask user to enter the given number for find the pairs & store the value in variable name "sum"
	...
	static void findPairs(int arr[], int size, int sum)
	{
		...
		for (int i = 0; i < size; i++) {
			...
			for (int j = i+1; j < size; j++) {
				...
				//if the addition of arr[i] & arr[j] is same as the given number value then the condition is true
				if ((arr[i] + arr[j]) == sum) {
					/*	if the all conditions are true then we simply print the arr[i] & arr[j] value which are equivalent to the given number. 
						this is the following pair of the given number.
					*/					
