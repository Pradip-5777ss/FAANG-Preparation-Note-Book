...
public class MergeSort {
	...
	private static void merge(int[] arr, int size, int left, int mid, int right) {
		...
		/*
		 * Now check, the size of the left & right subarray is greatter than
		 * the index of the left & right subarray or not. 
		 * If this condition is true then we check, in the left sorted subarray
		 * the ith index value is less than equal to the right sorted subarray 
		 * jth index value or not.
		 * If this condition is ture then we take the element of the given index
		 * of the left subarray and place it in the new array & increment i value.
		 * otherwise, we take the element of the given index of the right subarray
		 * and place it in the new array & increment j value.
		 * And finally increment k value.
		 */
		while (i < n1 && j < n2) {
			if (L[i] <= R[j]) {
				arr[k] = L[i];
		...
		/*
		 * If the right subarray is exhausted then we copy all the elements of left subarray
		 */
		while (i < n1) {
			arr[k] = L[i];
			i++;
		...
		/*
		 * If the left subarray is exhausted then we copy all the elements of the right subarray
		 */
		while (j < n2) {
			arr[k] = R[j];
			j++;
