...
public class SinglyLinkedList {
	...
	public static class mySll<E> {
		...
		public void createNewNode(E data) {
			...
			//if this condition is true then we insert the reference of the new node in the head variable.
			if (head == null) {
				head = createNewNode;
			   	return;
			}
			...
			//run a loop & check the condition upto it is not false. and we store the temp.next value in temp.
		...
		public void search(E data) {
			 ...
			 * [(while loop)...
			 * run the loop until the condition is not false.
			 * check the condition [temp.data == data] or not. if its true then we set the flag true.
			 * then increment the i variable value.
			 * increment the temp value. initialize the temp.next value in temp
			 * ] 
			 */
			if (head == null) {
		...
		public void deleteNode(int position) {
			...
			//if the position is 1 input by the user then the head value is updated by the [temp.next] value and delete the first value in the list.
			if (position == 1) {
				head = temp.next;
				return;
			}
